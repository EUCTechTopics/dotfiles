...
**Checklist:**
1. Create `~/.dotfiles/{app-name}/` directory (lowercase, hyphen-separated)
...
@$(REMOVE) {target_path}
@$(LINK) ~/.dotfiles/{app-dir}/{config_file} {target_path}
```
...
**Common Patterns:**
- **Single file**: `@$(LINK) ~/.dotfiles/app/config.json ~/.config/app/config.json`
- **Directory**: `@$(LINK) ~/.dotfiles/app ~/.config/app`
- **App Support**: `@$(LINK) ~/.dotfiles/app/config.plist ~/Library/Application\ Support/App/config.plist`
